...
// Переменные для дерева навыков
new g_vault_skills, g_sync_skills_hud
new g_health_lvl[33], g_speed_lvl[33], g_damage_lvl[33], g_skill_points[33]
#define MAX_SKILL_LVL 15
...
public plugin_init()
{
	...
	g_vault_skills = nvault_open("zp_skills_internal")
	g_sync_skills_hud = CreateHudSyncObj()
	set_task(0.5, "update_skills_hud", _, _, _, "b")
...
public client_putinserver(id)
{
	...
	if (!is_user_bot(id))
	{
		...
		if (get_pcvar_num(cvar_huddisplay))
			set_task(0.8, "ShowHUD", id+TASK_SHOWHUD, _, _, "b")
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if (!g_zombie[attacker])
	{
	if (random_num(1, 100) <= 20) { // Шанс 20%
	g_skill_points[attacker]++
	client_print(attacker, print_center, "Вы получили очко навыков!")
	}
}
...
public menu_game(id, key)
{
	...
	switch (key)
	{
		...
		} // <- Убедитесь, что эта скобка закрывает case 0
		case 1: // Extra Items
		{
			// Extra items enabled?
			if (get_pcvar_num(cvar_extraitems))
...
public ShowHUD(taskid)
{
	...
	if (!g_isalive[ID_SHOWHUD]) // Если игрок мертв (наблюдатель)
	{
		set_hudmessage(255, 255, 255, -1.0, 0.85, 0, 0.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L: %s^n%L: %d | %L: %s | %L: %d", ID_SHOWHUD, "HUD_SPECT", g_playername[id], ID_SHOWHUD, "HUD_HP", pev(id, pev_health), ID_SHOWHUD, "HUD_CLASS", class, ID_SHOWHUD, "HUD_AMMO", g_ammopacks[id])
	}
	else // Если игрок жив
	{
		set_hudmessage(red, green, blue, -1.0, 0.90, 0, 0.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L: %d | %L: %s | %L: %d", ID_SHOWHUD, "HUD_HP", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "HUD_CLASS", class, ID_SHOWHUD, "HUD_AMMO", g_ammopacks[ID_SHOWHUD])
	}
...
public show_skill_menu(id) {
    ...
    new menu = menu_create("\yНавыки выжившего", "menu_skills_handler")
    new info[128], cost
    
    // Живучесть
    cost = (g_health_lvl[id] / 5) + 1
    formatex(info, charsmax(info), "Живучесть [%d/%d] \y(%d очк.)", g_health_lvl[id], MAX_SKILL_LVL, cost)
    menu_additem(menu, info, "1")

    // Ловкость
    cost = (g_speed_lvl[id] / 5) + 1
    formatex(info, charsmax(info), "Ловкость [%d/%d] \y(%d очк.)", g_speed_lvl[id], MAX_SKILL_LVL, cost)
    menu_additem(menu, info, "2")

    // Сила
    cost = (g_damage_lvl[id] / 5) + 1
    formatex(info, charsmax(info), "Сила [%d/%d] \y(%d очк.)", g_damage_lvl[id], MAX_SKILL_LVL, cost)
...
public menu_skills_handler(id, menu, item) {
    ...
    // Тут логика повышения g_health_lvl[id] и т.д. аналогично прошлому коду
    // Не забываем вызывать save_skills(id)
    menu_destroy(menu);
    return PLUGIN_HANDLED;
}

public update_skills_hud() {
    for (new id = 1; id <= 32; id++) {
        if (is_user_alive(id) && !g_zombie[id]) {
            set_hudmessage(255, 215, 0, 0.02, 0.13, 0, 0.0, 0.6, 0.0, 0.0, -1)
            ShowSyncHudMsg(id, g_sync_skills_hud, "Урон: +%d%% | Скорость: +%d%%^nЗдоровье: +%d HP | Очки: %d", 
            g_damage_lvl[id]*3, g_speed_lvl[id]*2, g_health_lvl[id]*20, g_skill_points[id])
        }
    }
}